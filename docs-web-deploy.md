...
- `CLOUDFLARE_D1_DATABASE_ID`: the D1 database ID for `api-key-get-endpoints`.

Do not commit these values to the repository.

## Cloudflare dashboard setup

1. In Cloudflare, create a D1 database named `api-key-get-endpoints`.
2. Copy its database ID into the GitHub secret `CLOUDFLARE_D1_DATABASE_ID`.
3. Create a Cloudflare API token and save it in GitHub as `CLOUDFLARE_API_TOKEN`.
...
The workflow installs dependencies, validates the Worker syntax, injects the D1 database ID into `wrangler.jsonc` inside the temporary GitHub runner, applies D1 migrations, and deploys the Worker.
