...
async function requestApiKey(name, request, env) {
  ...
  await env.DB.prepare(
    "INSERT INTO approval_requests (id, key_name, requester_ip, created_at, expires_at) VALUES (?, ?, ?, ?, ?)"
  ).bind(id, name, requesterIp, now, expiresAt).run();
...
async function waitForApproval(env, id) {
  ...
  while (Date.now() < deadline) {
    const row = await env.DB.prepare("SELECT status FROM approval_requests WHERE id = ?").bind(id).first();
...
async function markExpiredOrRejected(env, id) {
  const now = Math.floor(Date.now() / 1000);
  await env.DB.prepare(
    "UPDATE approval_requests SET status = CASE WHEN expires_at <= ? THEN 'expired' ELSE 'rejected' END WHERE id = ? AND status = 'pending'"
  ).bind(now, id).run();
}
...
async function handleApprovalCallback(callbackQuery, env) {
  ...
  if (!requestRow || requestRow.status !== "pending" || requestRow.expires_at < now) {
